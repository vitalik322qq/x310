...
async def reset_menu(call: CallbackQuery):
    await call.answer()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🔁 Завершить у всех', callback_data='reset_all')],
        [InlineKeyboardButton(text='🔍 Завершить по ID', callback_data='reset_by_id')],
    ])
...
async def reset_all(call: CallbackQuery, state: FSMContext):
    cursor = conn.execute(
        'UPDATE users SET free_used=?, trial_expired=1',
        (TRIAL_LIMIT,)
    )
    conn.commit()
    ...
    await call.answer()
    await call.message.answer(f"✅ Триал завершён у всех ({count} пользователей).")
    await state.clear()

@dp.callback_query(F.data=='reset_by_id')
async def reset_by_id(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await call.message.answer('🆔 Введите ID для завершения триала:')
...
async def do_reset_by_id(msg: Message, state: FSMContext):
    ...
    uid = int(msg.text)
    cursor = conn.execute(
        'UPDATE users SET free_used=?, trial_expired=1 WHERE id=?',
        (TRIAL_LIMIT, uid)
    )
    conn.commit()
    count = cursor.rowcount
    ...
    if count:
        await msg.answer(f"✅ Триал завершён для пользователя {uid}.")
    else:
