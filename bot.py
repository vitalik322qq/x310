...
async def start_handler(message: Message):
    ...
    await message.answer(f"👾 Welcome to n3лох!\n{welcome}", reply_markup=sub_keyboard())
...
async def admin_menu(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton('🔄 Reset Trial',   callback_data='reset_trial')]
    ])
...
# Search Handler (only when not in FSM)
@dp.message(~F.text.startswith('/'), state=None)
async def search_handler(message: Message):
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"https://api.usersbox.ru/v1/search?q={q}",
...
"""
    ...
    await message.answer_document(FSInputFile(tmp_path, filename=f"{q}.html"))(FSInputFile(tmp_path, filename=f"{q}.html"))
    try:
        os.remove(tmp_path)
    except:
        pass
    ...
    if inv.get('status') == 'paid' and inv.get('payload'):
        ...
        if parts[0] == 'pay' and len(parts) >= 4:
            ...
            if plan == 'hide_data':
                ...
                subs = now + days * 86400
                c.execute('INSERT INTO users(id,subs_until,free_used) VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET subs_until=excluded.subs_until',
                          (uid, subs, 0))
