...
async def start_handler(message: Message):
    ...
    c.execute(
        'INSERT OR IGNORE INTO users(id,subs_until,free_used,hidden_data) VALUES(?,?,?,?)',
        (uid, 0, 0, 0)
    )
...
async def status_handler(message: Message):
    uid = message.from_user.id
    subs_until, free_used, hidden_data, requests_left, trial_expired = c.execute(
    ...
    sub = datetime.fromtimestamp(subs_until).strftime('%Y-%m-%d') if subs_until > now else 'none'
    free = 0 if trial_expired else TRIAL_LIMIT - free_used
